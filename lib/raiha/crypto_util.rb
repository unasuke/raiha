...
module Raiha::CryptoUtil
  ...
  # @param hash [String] hash algorithm name (default: "SHA256")
  # @return [String] output keying material with label
  def hkdf_expand_label(secret, label, context, length, hash: "SHA256")
    info = [length].pack("n")
    info += "tls13 #{label}".length.chr + "tls13 #{label}"
    info += context.length.chr + context
    hkdf_expand(secret, info, length, hash: hash)
  end
  ...
  def hkdf_expand(prk, info, length, hash: "SHA256")
