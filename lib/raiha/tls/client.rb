  ...
  module TLS
    class Client < Peer
      ...
      def receive_hello_retry_request(handshake, raw_bytes)
        hrr = handshake.message #: Handshake::ServerHello
      ...
      def build_client_hello
        ...
        ch_msg = hs_clienthello.message #: Handshake::ClientHello
        ch_msg.setup_key_share(@pkeys)
        @additional_extensions.each { |ext| ch_msg.extensions << ext }
        @client_hello = ch_msg
        @transcript_hash.digest_algorithm = ch_msg.cipher_suites.first.hash_algorithm
        ...
        if psk_entry
          ...
          if ticket_allows_early_data?(psk_entry)
            ch_msg.extensions << Handshake::Extension::EarlyData.new(on: :client_hello)
      ...
      private def setup_early_data_cipher(psk_entry)
        client_hello = @client_hello #: Handshake::ClientHello
      ...
      private def add_psk_to_client_hello(hs_clienthello, psk_entry)
        client_hello = hs_clienthello.message #: Handshake::ClientHello
        ...
        truncated = serialized[0...(serialized.bytesize - binders_size)] or
          raise Raiha::TLS::Error, "TODO: ClientHello serialize shorter than binders size"
      ...
      private def send_client_certificate
        ...
        if client_certificate
          cert_msg.certificate_entries << Handshake::Certificate::CertificateEntry.new(
            opaque_certificate_data: client_certificate.to_der,
            extensions: []
          )
        end
      ...
      private def rebuild_client_hello_for_retry(hrr, requested_group:)
        ...
        hs_clienthello = Raiha::TLS::Handshake.new.tap do |hs|
          ...
          hs.message = Raiha::TLS::Handshake::ClientHello.build.tap do |ch|
            ...
            sg_ext = ch.extensions.find { |e| e.is_a?(Handshake::Extension::SupportedGroups) } #: Handshake::Extension::SupportedGroups?
        ...
        ch_msg = hs_clienthello.message #: Handshake::ClientHello
        ch_msg.setup_key_share(retry_pkeys)
        @client_hello = ch_msg
      ...
      private def valid_server_hello?
        sh = @server_hello #: Handshake::ServerHello
        ch = @client_hello #: Handshake::ClientHello
        return false unless sh.legacy_session_id_echo == ch.legacy_session_id
        return false unless ch.cipher_suites.map(&:name).include?(sh.cipher_suite.name)

        return false unless sh.extensions.any? { |ext|
        ...
        has_key_share = sh.extensions.any? { |ext| ext.is_a?(Raiha::TLS::Handshake::Extension::KeyShare) }
        has_pre_shared_key = sh.extensions.any? { |ext| ext.is_a?(Raiha::TLS::Handshake::Extension::PreSharedKey) }
      ...
      private def setup_key_schedule
        sh = @server_hello #: Handshake::ServerHello
        server_key_share = sh.key_share #: Handshake::Extension::KeyShare
        @key_schedule.cipher_suite = sh.cipher_suite
      ...
      private def psk_mode?
        ...
        sh.extensions.any? { |ext| ext.is_a?(Handshake::Extension::PreSharedKey) }
      end

      private def verify_certificate_verify(certificate_verify)
        cv = certificate_verify.message #: Handshake::CertificateVerify
        raise unless cv.verify_signature(
      ...
      private def verify_finished(finished)
        ...
        raise unless fin.verify_data == @key_schedule.finished_verify_data(@transcript_hash.hash, from: :server)
      end
      ...
      private def save_to_sslkeylogfile
        ...
        ch = @client_hello #: Handshake::ClientHello
        body = <<~SSLKEYLOGFILE
          SERVER_HANDSHAKE_TRAFFIC_SECRET #{ch.random.unpack1("H*")} #{@key_schedule.server_handshake_traffic_secret.unpack1("H*")}
          SERVER_TRAFFIC_SECRET_0 #{ch.random.unpack1("H*")} #{@key_schedule.server_application_traffic_secret[0].unpack1("H*")}
          CLIENT_HANDSHAKE_TRAFFIC_SECRET #{ch.random.unpack1("H*")} #{@key_schedule.client_handshake_traffic_secret.unpack1("H*")}
          CLIENT_TRAFFIC_SECRET_0 #{ch.random.unpack1("H*")} #{@key_schedule.client_application_traffic_secret[0].unpack1("H*")}
        SSLKEYLOGFILE
